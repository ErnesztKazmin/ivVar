...
#' @export
Psi.varest <- function (x, nstep = 10, instrument, instrumented){
  if (!is(x, "varest")) {
  ...
  res_model_hat <- first_stage(x, instrument, instrumented)
  coefs <- second_stage(x, res_model_hat, instrumented)
  P <- sigma.u
  P[] <- 0
  P[, instrumented]
